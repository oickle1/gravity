...
public class PendulumRunner {
	...
	public static void main(String[] args) {
		...
		//instantiation of Earth's gravity
		GravityConstant loGrav = new GravityConstant(9.81);
		GravityConstant hiGrav = new GravityConstant(25);
		
		double delta = (args.length == 0) ? .1 : Double.parseDouble(args[0]);
		double sLen = 10, pMass = 10, theta0 = Math.PI / 30;
		RegularPendulum rp = new RegularPendulum(sLen, pMass, theta0, delta, loGrav);
		SimplePendulum sp = new SimplePendulum(sLen, pMass, theta0, loGrav);
		RegularPendulum rpCoarse = new RegularPendulum(sLen, pMass, theta0, .1, hiGrav);
